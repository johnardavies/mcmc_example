...
def mcmc_threshold(x, y, data_sample):
    ...
    y is the proposed new s	tate
    norm.pdf(x,y,100) is the function x of a normal distribution with mean y and variance 100
    """
    ratio = (posterior(y, 0.3, 0.1, data_sample) * norm.pdf(x, y, 100)) / (
        posterior(x, 0.3, 0.1, data_sample) * norm.pdf(y, x, 100)
    )
...
for index, sample_size in enumerate(data_sets):
    ...
    for i in range(0, sample_number):
        ...
        if i % 10000 == 0:
            ...
            plt.title(
                "Distribution of mcmc chain's " + str(i) + "'th run", weight="bold"
            )
    ...
    axs[index].hist(
        data_store["state_of_chain"], density=True, bins=50, label="mcmc distribution"
    )
    axs[index].legend()
    axs[index].set_title(
        f"Distribution of mcmc chain's {i}'th run based on {sample_size} datapoints",
        weight="bold",
    )
